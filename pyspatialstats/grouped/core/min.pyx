...
cdef double* _grouped_min(size_t[:] ind, double[:] v, size_t max_ind) except NULL nogil:
    cdef:
...
def grouped_min_npy(size_t[:] ind, double[:] v) -> np.ndarray[tuple[int], np.float64]:
    cdef:
        size_t max_ind
        double* r
...
def grouped_min_npy_filtered(size_t[:] ind, double[:] v) ->  np.ndarray[tuple[int], np.float64]:
    cdef:
