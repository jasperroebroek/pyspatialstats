...
cdef double* _grouped_std(size_t[:] ind, double[:] v, double *mean_v, size_t max_ind) except NULL nogil:
    cdef:
...
def grouped_std_npy(size_t[:] ind, double[:] v) -> np.ndarray[tuple[int], np.float64]:
    cdef:
        double *mean_v, *r
        size_t max_ind
...
def grouped_std_npy_filtered(size_t[:] ind, double[:] v) -> np.ndarray[tuple[int], np.float64]:
    cdef:
