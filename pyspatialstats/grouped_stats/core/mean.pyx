...
from pyspatialstats.stats.mean cimport _bootstrap_mean, CyBootstrapMeanResult
from pyspatialstats.results import BootstrapMeanResult


cdef double* _grouped_mean(size_t[:] ind, double[:] v, size_t max_ind) nogil:
    cdef:
...
def grouped_mean_npy_filtered(size_t[:] ind, double[:] v) -> np.ndarray:
    cdef:
...
def grouped_mean_bootstrap_npy_filtered(size_t[:] ind, double[:] v, n_bootstraps: int, seed: int) -> np.ndarray:
    cdef:
        size_t i, max_ind, c = 0, num_inds = 0
        size_t *count_v
        double *r_mean_v
